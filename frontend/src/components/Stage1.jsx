...
/* Side-by-side comparison view used by the adaptive "divided" layout (RQ3):
   all responses rendered simultaneously so users can compare diverging views. */
function Stage1CompareGrid({ responses }) {
  return (
    <div className="stage1-compare-grid">
      {responses.map((resp, index) => {
        ...
        return (
          <div key={index} className="stage1-compare-card glass-panel">
            <div className="model-header">
              <div className="model-identity">
                <span className="model-avatar" style={{ backgroundColor: resp?.error ? '#ef4444' : visuals.color }}>
                  {visuals.icon}
                </span>
                <div className="model-info">
                  <span className="model-name-large">{shortName}</span>
                  <span className="model-provider-badge" style={{ borderColor: visuals.color, color: visuals.color }}>
                    {visuals.name}
                  </span>
                </div>
              </div>
            ...
            {resp?.error ? (
              <div className="response-error">
                ...
                <div className="error-details">
                  ...
                  <div className="error-message">{resp?.error_message || 'Unknown error'}</div>
                </div>
              </div>
            ) : (
              <div className="response-text markdown-content">
                <ThinkBlockRenderer
                  content={typeof resp.response === 'string' ? resp.response : String(resp.response || 'No response')}
                />
              </div>
            )}
          </div>
...
export default function Stage1({ responses, startTime, endTime, sideBySide = false }) {
