...
import ConsensusBadge, { ConfidenceScore, DividedBanner } from './ConsensusBadge';
import UIConditionToggle from './UIConditionToggle';
...
export default function ChatInterface({
    ...
    // RQ2/RQ3 experimental interface condition: static | badge | adaptive
    const [uiCondition, setUiCondition] = useState(
        () => localStorage.getItem('ai_counsel_ui_condition') || 'static'
    );
    const handleUiConditionChange = (condition) => {
        setUiCondition(condition);
        localStorage.setItem('ai_counsel_ui_condition', condition);
    };
                ...
                {!councilConfigured ? (
                    ...
                    <form className="input-container" onSubmit={handleSubmit}>
                        ...
                        <div className="input-row-bottom">
                            ...
                            <UIConditionToggle
                                value={uiCondition}
                                onChange={handleUiConditionChange}
                            />
                        </div>
                    </form>
                )}
...
function CouncilMessageRenderer({
    ...
    // RQ3 full-reconfiguration: council deliberation collapsed by default under consensus
    ...
    // --- RQ2/RQ3: agreement-driven adaptive interface ---
    // Reconfiguration only applies once the run is complete so the streaming
    // experience (and its scroll anchors) is identical across conditions.
    const agreement = displayMetadata.agreement || null;
    const runComplete = Boolean(displayStage3)
        && !msg.loading?.stage1 && !msg.loading?.stage2
        && !msg.loading?.stage3 && !msg.loading?.stage4;
    const adaptiveActive = uiCondition === 'adaptive' && agreement && runComplete;
    const consensusLayout = adaptiveActive && agreement.council_state === 'consensus';
    const dividedLayout = adaptiveActive && agreement.council_state === 'divided';
    const hideDeliberation = consensusLayout && !showDeliberation;
    const showMinimalBadge = agreement
        && (uiCondition === 'badge' || (uiCondition === 'adaptive' && !adaptiveActive));
    const flowClass = [
        'council-message-flow',
        consensusLayout ? 'layout-consensus' : '',
        dividedLayout ? 'layout-divided' : '',
    ].filter(Boolean).join(' ');
    ...
    return (
        <div className={flowClass}>
            ...
            {/* RQ3 full reconfiguration: consensus summary / divided warning */}
            {consensusLayout && (
                <div className="consensus-summary">
                    <ConsensusBadge agreement={agreement} />
                    <ConfidenceScore agreement={agreement} />
                </div>
            )}
            {dividedLayout && <DividedBanner agreement={agreement} />}

            {/* Stage 1: Council Grid (during active round deliberation only) */}
            {shouldShowStage1CouncilGrid(msg) && (
                <div className={`stage-container ${hideDeliberation ? 'deliberation-hidden' : ''}`}>
            ...
            {/* Stage 1 Content */}
            {showStage1 && (
                <div className={hideDeliberation ? 'deliberation-hidden' : ''}>
            ...
            <div
                ...
                className={`stage-scroll-anchor stage2-anchor ${hideDeliberation ? 'deliberation-hidden' : ''}`}
            >
            ...
            {/* RQ2 minimal adaptation: consensus badge, layout otherwise unchanged */}
            {showMinimalBadge && <ConsensusBadge agreement={agreement} />}

            {/* Stage 3 */}
            <div
                ref={isActiveCouncilTurn ? stage3AnchorRef : null}
                className="stage-scroll-anchor stage3-anchor"
            >
                {dividedLayout && (
                    <div className="synthesis-note">
                        ⚖️ The final answer below is an attempt to balance the council's diverging views.
                    </div>
                )}
            ...
            {/* RQ3 full reconfiguration: deliberation collapsed under consensus */}
            {consensusLayout && (
                <div className="deliberation-toggle-row">
                    <button
                        className="deliberation-toggle-btn"
                        onClick={() => setShowDeliberation((v) => !v)}
                    >
                        {showDeliberation
                            ? '▴ Hide council deliberation'
